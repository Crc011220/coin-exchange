...
public class DisruptorAutoConfiguration {
     ...
     * waitStrategy : 等待策略: 当ringBuffer 没有数据时,我们怎么等待
     */
    @Bean
    public RingBuffer<OrderEvent> ringBuffer(
        ...
        disruptor = new Disruptor<>(eventFactory,
                disruptorProperties.getRingBufferSize(), threadFactory, producerType, waitStrategy) ;
        disruptor.setDefaultExceptionHandler(new DisruptorHandlerException());

        // 设置消费者---每个消费者代表一个交易对,有多少个交易对,就有多少个eventHandlers ,
