...
import com.rc.model.Line;
import com.rc.util.KlineTimeUtil;
...
public class TradeKlineService implements CommandLineRunner {
    ...
    private void generateKLine(CreateKLineDto klineData, KlineType klineType) {
        // 1 获取之前该K 线的数据
        String redisKey = Constants.REDIS_KEY_TRADE_KLINE + klineData.getSymbol().toLowerCase() + ":"
                + klineType.getValue().toLowerCase();
        ...
        // 4 若当前的时间: 还在上一个时间的区间内
        assert dateTime != null;
        if (dateTime.compareTo(historyKline.getTime()) > 0) {
            ...
            // 最高价 ,最低价
            if (klineData.getPrice().compareTo(historyKline.getHigh()) > 0) {
                line.setHigh(klineData.getPrice());
                line.setLow(historyKline.getClose());
            }
            if (klineData.getPrice().compareTo(historyKline.getLow()) > 0) {
